...
var numShow = 0; // The number of active forms that will be shown on the visualization.
...
var numVisExecuted = 0; // The number of visualizations executed since the page was loaded.
var currentVisID; // The ID of the visualization being executed.
var numProfiles = 0; // The number of profiles available

/**
 * Call back function that gets the sources from the client,
 * puts them in the global sources array, and creates the initial
 * source form.
...
function putSources(data) {
    ...
    sources = new Array();
    for(var x in data['Sources']['Source']) {
        
        // Get the source name
        var source = data['Sources']['Source'][x]['Name'];
        sources.push({
            sourceName: source 
        });
    }
...
/**
 * Gets all of the available sources from the WattDepot server
 * or produces an error if the Ajax call fails.
 */
function getSources() {
    if(wdClient == null) {
...
/**
 * Loads the WattDepot client to the server and gets the
 * sources from the server.  To be called onload.
 */
function loadServer() {
    ...
    $('#shDefault').append('<li><a onclick="addrClick(\'' + defaultServer + '\')">' + defaultServer + '</a></li>');
    seedServerHistory();
    ...
    if(proNum != null) {
        ...
        for(var i = 0; i < proNum; i++) {
            ...
            $('#profileSelect').append('<option value="'+decodeURIComponent(link)+'">'+decodeURIComponent(name)+'</option>');
            numProfiles++;
        }
...
/**
 * Pads the given value with a leading zero if it positive and
 * is less than 10.
 *
 * @param value The value to pad.  Should be a positive integer.
 *
 ...
 *
 * @param value The value to generate an option string for.  Should
...
function getDate(id, index) {
    ...
    if(!timePattern.test(time)){
        ...
        $('#'+id+'TimePicker'+index).wrap('<a href="#" rel="tooltip" data-placement="top" title="Error: Time must be in the format (HH:MM:SS)">');
    }
    ...
    if(!datePattern.test(date)){
        ...
        $('#'+id+'Datepicker'+index).wrap('<a href="#" rel="tooltip" data-placement="top" title="Error: Date must be in the format (MM/DD/YYYY)">');
    }
    if(error) {
        return null;
    }
 ...
 *
 * @param seconds The number of seconds in the time string.
 */
 ...
 * visualize subquery.
 *
 * @param response The response from the visualizer query.
 *
 * @param queryIndex The index of the query in the dataQueries array.
 *
 * @param startQueryTime The time at which the query was sent from the Date's getTime() method.
 * 
 * @param queryID The ID of the query that this response is for.
 */
function visualizerResponse (response, queryIndex, startQueryTime, queryID){
        ...
        for(i = 1; i < numColumns; i++) {
            for(var j = 0; j < numRows; j++) {
                ...
                if(matchingRows.length < 1) {
                    ...
                    dataQueries[queryIndex].queryResults.setValue(newRow, 0, table.getValue(j, 0));
                    dataQueries[queryIndex].queryResults.setValue(newRow, i, table.getValue(j, i));
                }
                
                // Else update matching rows
                else {
                    for(var l = 0; l < matchingRows.length; l++) {
                        dataQueries[queryIndex].queryResults.setValue(matchingRows[l], i, table.getValue(j, i));
                    }
                }
            }
        }
    ...
    if(averageTimePerPoint == 0) {
        ...
        averageTimePerPoint = (averageTimePerPoint + timePerPoint) / totalNumQueriesReturned;
    }
   
    // Update visuals.
    $('#visualizeProgressBar').width((numDataPointsRetrieved / totalNumPoints) * 100 + "%");
    $('#progressLabel').empty();
    $('#progressLabel').append("Getting data... "+ Math.round(numDataPointsRetrieved) + " / " + Math.round(totalNumPoints) + " completed.");
    $('#timeLabel').empty();
    $('#timeLabel').append("Estimated Time Remaining: " + createTimeString(Math.round((averageTimePerPoint * (totalNumPoints - numDataPointsRetrieved)))) + ".");
               
    // If all of the queries for the source have returned, pus the data to the completed data array.
    ...
    if(dataArray.length >= 1) {
        ...
        for(i = 1; i < dataArray.length; i++) {
            ...
            for(j = 1; j < dataArray[i].getNumberOfColumns(); j++) {
                newColumn = mergedTable.addColumn(dataArray[i].getColumnType(j), dataArray[i].getColumnLabel(j));
                for(var k = 0; k < dataArray[i].getNumberOfRows(); k++) {
                    ...
                    if(matchingRows.length < 1) {
                        ...
                        mergedTable.setValue(newRow, newColumn, dataArray[i].getValue(k, j));
                    }
                    else {
                        for(l = 0; l < matchingRows.length; l++) {
                            mergedTable.setValue(matchingRows[l], newColumn, dataArray[i].getValue(k, j));
                        }
                    }
                }
            }
        ...
        var chart = new google.visualization.AnnotatedTimeLine(document.getElementById('chartDiv'));
 ...
 *
 * @param source The source the query is getting data for.
 * 
 * @param dataType The type of data the query is retrieving.
 * 
...
function splitQuery(server, source, dataType, startTime, endTime, interval) {
    ...
    while(tempTime < endTime) {
        query = new google.visualization.Query(server +'sources/'+ source +'/gviz/calculated?startTime='+wdClient.getTimestampFromDate(startTime)+'&endTime='+wdClient.getTimestampFromDate(tempTime)+'&samplingInterval='+interval);
    ...
    query = new google.visualization.Query(server +'sources/'+ source +'/gviz/calculated?startTime='+wdClient.getTimestampFromDate(startTime)+'&endTime='+wdClient.getTimestampFromDate(endTime)+'&samplingInterval='+interval);
    query.setQuery('select timePoint, ' + dataType);
    query.setTimeout(queryTimeOut);
    returnArray.push(query);
    return returnArray;
...
function visualize() {
    ...
    for(var loopIndex = 0; loopIndex < activeIndex.length; loopIndex++) {
        ...
        if(activeIndex[loopIndex].sourceSelected) {
            ...
            if(findSource(source) != -1) {
                ...
                error = true;
                $('#sourceSelect'+formIndex+"_chzn").wrap('<a href="#" rel="tooltip" data-placement="top" title="Error: Source is not a valid source on the current server." />');
                $('#sourceSelect'+formIndex+"_chzn").addClass('invalid');
            }
        }
        else {
            error = true;
            $('#sourceSelect'+formIndex+"_chzn").wrap('<a href="#" rel="tooltip" data-placement="top" title="Error: No source has been selected." />');
            $('#sourceSelect'+formIndex+"_chzn").addClass('invalid');
        }
        ...
        if((endTime - startTime) / 60000 < interval) {
            ...
            $('#interval'+formIndex).wrap('<a href="#" rel="tooltip" data-placement="top" title="Error: Interval is larger than the data range.  Please chooose a smaller interval." />');
        }
        ...
        switch(dataType) {
            ...
            case "carbonEmitted":
                ...
                if(!error) {
                    dataQueries.push({
                        'source':source, 
                        'numReturned':0,
                        'queries':splitQuery(server, source, dataType, startTime, endTime, interval), 
                        'queryResults':null
                    });
                }
                break;
            default:
                ...
                if(typeof dataType === "undefined") {
                    $('#dataType'+formIndex).wrap('<a href="#" rel="tooltip" data-placement="top" title="Error: The data type to retrieve was not selected." />');
                }
                else {
                    $('#dataType'+formIndex).wrap('<a href="#" rel="tooltip" data-placement="top" title=""Error: Data type ' + dataType + ' is not supported." />');
                }
    ...
    for(loopIndex = 0; loopIndex < dataQueries.length; loopIndex++) {
        for(var loopIndex2 = 0; loopIndex2 < dataQueries[loopIndex].queries.length; loopIndex2++) {
            sendQuery(dataQueries[loopIndex].queries[loopIndex2], loopIndex, new Date().getTime(), currentVisID);
        }
    }
}

/**
 * Finds the index of the object in sources that
 * has the given name..
 * 
...
/**
 * Fills the dataType options and selects the given dataType.  To be passed
 * as a parameter to sourceChange.
 * 
 * @param data The data returned from the AJAX query for the latest sensor data.
 * 
 * @param formIndex The index of the form to modify.
 * 
 * @param dataType The dataType to automatically select.
 */
...
function putDataTypes(data, formIndex) {
    ...
    var properties = data['SensorData']['Properties']['Property'];  
    for(var i in properties) {
        switch(properties[i]["Key"]){
            case "powerConsumed":
                $('#dataType' + formIndex).append('<option value="powerConsumed">Power Consumed</option>');
                break;
            case "powerGenerated":
                $('#dataType' + formIndex).append('<option value="powerGenerated">Power Generated</option>');
                break;
            case "energyConsumedToDate":
                $('#dataType' + formIndex).append('<option value="energyConsumed">Energy Consumed</option>');
                break;
            case "energyGenerated":
                $('#dataType' + formIndex).append('<option value="energyGenerated">Energy Generated</option>');
                break;
            case "carbonGenerated":
                $('#dataType' + formIndex).append('<option value="carbonGenerated">Carbon Generated</option>');
                break;
 ...
 * 
 * @param formIndex The index of the form of the sourceSelect to handle.
 * 
 * @param dataTypeGenerator A function that takes either two parameters (data, formIndex)
 *                          or three parameters (data, formIndex, selectedDataType) that
 *                          generates the dataType options and possibly other actions.
 *                          
 * @param selectedDataType The dataType that should be automatically selected.  Set to null
 *                         if none should be selected.
 */
function sourceChange(formIndex, dataTypeGenerator, selectedDataType) {
    ...
    if(selectedDataType == "") {
        wdClient.getSourceLatestSensorData($('#sourceSelect'+formIndex).val(), function(data) {
            dataTypeGenerator(data, formIndex);
        });
    }
    else {
        wdClient.getSourceLatestSensorData($('#sourceSelect'+formIndex).val(), function(data) {
    ...
    //----------------------------
    //
    //Note: made changes to the: $('#visualizerFormsDiv').append() to account for ids and layout.
    //
    //marking off work for flsendata
    //
    //This will be called(along with above code) whenever a source is changed.
    //it calls get first sensor function (located on index.html) sending the name of the source and the name of the div id.
    getFirstSensor($('#sourceSelect'+formIndex).val(), '#earliestSensor' + formIndex);
    //Latestsensor functioncall
    //same idea, send the name of the source and the div id.
    getLatestSensor($('#sourceSelect'+formIndex).val(), '#latestSensor' + formIndex);
//end mark off for flsendata
//----------------------------
}

/**
 * Inserts the HTML for a new visualizer form into the DOM with the given
 * index.
 * 
 * @param formIndex The index of the form being created.
 */
function insertVisualizerFormHTML(formIndex) {
    var currentDate = new Date();
    $('#visualizerFormsDiv').append(
        '<div class="row-fluid form" id="form'+formIndex+'">'
        +'<div class="row-fluid">'
        +'<div id="sourceDiv'+formIndex+'" class="span2 control-group"><div class="span2"><a href="#" rel="tooltip" data-placement="right" title="Show / Hide the source."><input type="checkbox" class="span11" id="show'+formIndex+'" /></a></div><div class="span10"><select id="sourceSelect'+formIndex+'" class="span12 chzn-select" data-placeholder="Choose a source..."><option value="" selected></option></select></div></div>'
        +'<div id="dataTypeDiv'+ formIndex +'" class="span2 control-group"><select class="span12" id="dataType'+formIndex+'"></select></div>'
        +'<div class="span7" id="timeDiv'+formIndex+'"><div class="span5 control-group"><div class="input-append bootstrap-timepicker-component span5"><input type="text" id="startTimePicker'+formIndex+'" class="timepicker-default input-small"><span id="startTimePickerSpan'+ formIndex+ '" class="add-on"><i class="icon-time"></i></span></div><div class="span5"><input class="span12" type="text" value="'+(currentDate.getMonth()+1)+'/'+(currentDate.getDate())+'/'+currentDate.getFullYear()+'" id="startDatepicker'+formIndex+'"></div><div class="span12 boundaryData"><div class="span5">Earliest Sensor:</div><div class="span6" id="earliestSensor' + formIndex + '"></div></div></div>'
        +'<div class="span5 control-group"><div class="input-append bootstrap-timepicker-component span5"><input type="text" id="endTimePicker'+formIndex+'" class="timepicker-default input-small"><span id="endTimePickerSpan'+ formIndex+ '" class="add-on"><i class="icon-time"></i></span></div><div class="span5"><input class="span12" type="text" value="'+(currentDate.getMonth() + 1)+'/'+currentDate.getDate()+'/'+currentDate.getFullYear()+'" id="endDatepicker'+formIndex+'"></div><div class="span2"><label class="checkbox"><input type="checkbox" id="endTimeNow' + formIndex + '" value="now">Now</label></div><div class="span12 boundaryData"><div class="span5">Latest Sensor:</div><div id="latestSensor' + formIndex + '" class="span6"></div></div></div>'
        +'<div class="span2 control-group"><select class="span12" id="interval'+formIndex+'"><option value="15">15 mins</option><option value="30">30 mins</option><option value="60">1 hr</option><option value="120">2 hr</option><option value="1440">1 Day</option><option value="10080">1 Week</option><option value="43200">1 Month</option><option value="129600">3 Months</option><option value="259200">6 Months</option><option value="518400">1 Year</option></select></div></div>'
        +'<div class="span1" id="removeDiv'+formIndex+'"><a href="#" rel="tooltip" data-placement="left" title="Remove the source."><button id="remove'+formIndex+'" class="btn"><i class="icon-minus-sign"></i></button></a></div></div></div></div>');
    ...
    for(var source in sources) {
        $('#sourceSelect'+formIndex).append('<option value="'+sources[source].sourceName+'">'+sources[source].sourceName+'</option>');
    }
...
/**
* Creates a new empty source form for the Visualizer and adds it
* to the DOM.
*/
 ...
 * 
 * @param show If show is checked or not (boolean true or false).
 * 
 * @param selectedSource The name of the source that should be selected (string).  "" indicates no source and no dataType will be selected as well.
 * 
 * @param dataType The value of the data type option that should be selected (string).  Ignored if selectedSource == "".
 * 
 * @param startTime The starting time (string "HH:MM:SS").
 * 
 * @param startDate The starting date (string "MM/DD/YYYY").
 * 
 * @param endTime The ending time (string "HH:MM:SS").
 * 
 * @param endDate The ending date (string "MM/DD/YYYY").
 * 
 * @param now If now is checked (boolean true or false).
 * 
 * @param interval The interval to poll data at (string "# of minutes").
 * 
 * @param dataTypeGenerator A function that handles the generation of he data type options and possibly other actions.
 */ 
function createFilledVisualizerForm(show, selectedSource, dataType, startTime, startDate, endTime, endDate, now, interval, dataTypeGenerator) {
    if(wdClient == null) {
        alert("Please select a server and submit first please!");
        return;
    }
...
function addVisualizerForm() {
    if(activeIndex.length == 0) {
        ...
        if(activeIndex[activeIndex.length - 1].sourceSelected) {
            createFilledVisualizerForm($('#show'+lastActiveFormIndex).is(':checked'), $('#sourceSelect'+lastActiveFormIndex+' option:selected').val(), $('#dataType'+lastActiveFormIndex+' option:selected').val(), $('#startTimePicker'+lastActiveFormIndex).val(), $('#startDatepicker'+lastActiveFormIndex).val(), $('#endTimePicker'+lastActiveFormIndex).val(), $('#endDatepicker'+lastActiveFormIndex).val(), $('#endTimeNow'+lastActiveFormIndex).is(':checked'), $('#interval'+lastActiveFormIndex+' option:selected').val(), putAndSelectDataTypes);
        }
        else {
            createFilledVisualizerForm($('#show'+lastActiveFormIndex).is(':checked'), "", "", $('#startTimePicker'+lastActiveFormIndex).val(), $('#startDatepicker'+lastActiveFormIndex).val(), $('#endTimePicker'+lastActiveFormIndex).val(), $('#endDatepicker'+lastActiveFormIndex).val(), $('#endTimeNow'+lastActiveFormIndex).is(':checked'), $('#interval'+lastActiveFormIndex+' option:selected').val(), putAndSelectDataTypes);
        }
    }
}

/**
 * Adds a new, user named profile.  Called on click by the newProfileButton (Save).
 */
function addNewProfile() {
    var link = $('#linkSpace').val();
    var name = $('#newProfileText').val();
    $('#profileSelect').append('<option value="'+link+'">'+name+'</option>');
    numProfiles++;
...
/**
 * Redirects the page to load the selected profile if it is not the
 * current one.  Called when the profileSelect changes.
 */ 
